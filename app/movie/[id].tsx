...
const Details = () => {
  ...
  useEffect(() => {
    const loadBookmarkState = async () => {
      ...
      try {
        const storedMovies = await AsyncStorage.getItem("bookmarked_movies");
        ...
        const savedMovies = Array.isArray(parsedMovies) ? parsedMovies : [];
        setIsBookmarked(
    ...
    <View className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        <View>
          ...
          <TouchableOpacity
            className="absolute top-5 right-5 rounded-full size-12 bg-black/50 flex items-center justify-center"
            onPress={handleBookmarkToggle}
          >
            <Image
              source={icons.save}
              className="size-6"
              tintColor={isBookmarked ? "#facc15" : "#ffffff"}
            />
          </TouchableOpacity>
        </View>
